...
from anima.ui.dialogs.progress_dialog import ProgressDialogManager
...
class ProgressDialogManagerTestCase(unittest.TestCase):
    ...
    def test_being_singleton(self):
        """ProgressDialogManager is a Singleton class."""
    ...
    def test_register_will_return_a_progress_caller_instance(self):
        """ProgressDialogManager.register() method will return a
    ...
    def test_register_will_store_the_given_caller(self):
        """ProgressWindow.register() method will store the given
    ...
    def test_register_will_create_the_window_if_it_is_not_created_yet(self):
        """register method will create the progressWindow if it
        is not created yet
        """
        pm = ProgressDialogManager()
    ...
    def test_step_will_increment_the_call_count_of_the_given_caller(self):
        """step method will increment the step of the caller"""
        pm = ProgressDialogManager()
        caller = pm.register(100)
        self.assertEqual(caller.current_step, 0)
    ...
    def test_step_removes_the_given_caller_if_it_is_at_maximum(self):
        """step method will automatically remove the caller from
    ...
    def test_step_will_step_the_progress_dialog(self):
        """step method will call QProgressDialog.setRange()
        properly
        """
        pm = ProgressDialogManager()
    ...
    def test_step_will_set_the_dialog_title(self):
        """step method will set the dialog title to the stepped caller."""
        pm = ProgressDialogManager()
        ...
        pm.step(caller1)
        self.assertEqual(
            pm.dialog.call_info["setLabelText"], [(f"{test_title1} : ",), {}]
        )

        pm.step(caller2)
        self.assertEqual(
            pm.dialog.call_info["setLabelText"], [(f"{test_title2} : ",), {}]
        )

    def test_end_progress_method_removes_the_given_caller_from_list(self):
        """end_progress method will remove the given caller from
        the callers list
        """
        pm = ProgressDialogManager()
        caller = pm.register(5)
    ...
    def test_end_progress_will_close_dialog_if_there_are_no_callers_left(self):
        """end_progress method will close the progress dialog
