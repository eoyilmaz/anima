...
class EDLImporterTestCase(unittest.TestCase):
    ...
    def test_edl_path_lineEdit_change_will_update_edl_preview_textEdit(self):
        """changing edl_path_lineEdit will update the
        ...
        self.assertEqual(
            self.dialog.edl_preview_plainTextEdit.toPlainText(), edl_content
        )

    def test_edl_path_lineEdit_will_clear_edl_preview_textEdit_if_path_invalid(self):
        """changing edl_path_lineEdit will clear the
        edl_preview_textEdit if edl_path_lineEdit is invalid
        """
        self.dialog.edl_preview_plainTextEdit.setPlainText("Some text")

        self.assertEqual(
            self.dialog.edl_preview_plainTextEdit.toPlainText(), "Some text"
        )

        self.dialog.edl_path_lineEdit.setText("Some text")

        self.assertEqual(self.dialog.edl_preview_plainTextEdit.toPlainText(), "")

    def test_send_pushButton_will_warn_the_user_to_set_the_media_files_path_if_it_is_not_set_before(
        self,
    ):
        """send_pushButton will warn the user to set the media files
        ...
        self.assertEqual("Media path doesn't exists", PatchedMessageBox.message)

    def test_send_pushButton_will_copy_mxf_files_to_media_files_folder(self):
        """send_pushButton will copy the mxf files to media files
        folder
        """
    ...
    #     """send_pushButton will show progress dialog
    #     """
    #     # create a proper edl file with proper files in a temp location
    #     # there should be MXF files
    #     tempdir = tempfile.gettempdir()
    ...
    def test_send_pushButton_will_open_the_edl_with_avid_edl_manager(self):
        """send_pushButton will open the edl with AVID EDL
        Manager
        """
        # Test Setup Idea
        #
    ...
    def test_media_files_path_lineEdit_content_will_be_stored(self):
        """media_files_path_lineEdit content will be stored in
        ...
        media_file_path_storage_full_path = os.path.join(
            defaults.local_cache_folder, defaults.avid_media_file_path_storage
        )
