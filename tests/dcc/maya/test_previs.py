...
class ShotSplitterTestCase(unittest.TestCase):
    """tests the anima.dcc.maya.previs.ShotSplitter class"""
    ...
    def setUp(self):
        ...
        self.user1 = User(
            name="User 1", login="User 1", email="user1@users.com", password="12345"
        )
        ...
        #
        self.assets = Task(
            name="Assets", project=self.project, responsible=[self.user1]
        )
        ...
        #
        self.sequences = Task(
            name="Sequences", project=self.project, responsible=[self.user1]
        )

        self.seq001 = Sequence(name="Seq001", code="Seq001", parent=self.sequences)

        self.scene_task = Task(name="001_IST", parent=self.seq001)

        self.scene_previs_type = Type(
            name="Scene Previs", code="Scene Previs", target_entity_type="Task"
        )

        self.scene_previs = Task(
            name="Scene Previs", parent=self.scene_task, type=self.scene_previs_type
        )

        self.shots = Task(name="Shots", parent=self.scene_task)

        self.shot1 = Shot(
            name="Seq001_001_IST_0010", code="Seq001_001_IST_0010", parent=self.shots
        )
