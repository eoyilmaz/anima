...
class RateTestCase(unittest.TestCase):
    ...
    def test_timebase_argument_skipped(self):
        """default value will be used when the timebase argument
        is skipped
        """
        r = Rate()
    ...
    def test_timebase_argument_is_None(self):
        """timebase attribute value will be set to the default
    ...
    def test_timebase_argument_is_not_a_str_value(self):
        """a TypeError will be raised when the timebase argument is
        not a string value
        """
        with self.assertRaises(TypeError) as cm:
            Rate(timebase=24)

        self.assertEqual(cm.exception.message, "Rate.timebase should be a str, not int")

    def test_timebase_attribute_is_not_a_str(self):
        """a TypeError will be raised when the timebase attribute is
        not set to a str
        """
        r = Rate(timebase="12")
        with self.assertRaises(TypeError) as cm:
    ...
    def test_timebase_argument_is_working_properly(self):
        """timebase argument value is correctly passed to the
        timebase attribute
        """
        r = Rate(timebase="12")
    ...
    def test_timebase_attribute_is_working_properly(self):
        """timebase attribute is working properly"""
        r = Rate(timebase="12")
        r.timebase = "15"
        self.assertEqual("15", r.timebase)

    def test_ntsc_argument_is_skipped(self):
        """ntsc attribute will be False if the ntsc argument is
        skipped
        """
        r = Rate()
    ...
    def test_ntsc_attribute_is_set_to_None(self):
        """ntsc attribute value will be False if it is set to
        None
        """
        r = Rate(ntsc=True)
    ...
    def test_ntsc_argument_is_not_a_bool_value(self):
        """a TypeError will be raised when the ntsc argument is not
        a bool
        """
        with self.assertRaises(TypeError) as cm:
            r = Rate(ntsc="not a bool value")

        self.assertEqual(str(cm.exception), "Rate.ntsc should be a bool value, not str")

    def test_ntsc_attribute_is_set_to_a_non_bool_value(self):
        """a TypeError will be raised when the ntsc attribute is set
        to a non bool value
        """
        r = Rate(ntsc=True)
        with self.assertRaises(TypeError) as cm:
    ...
    def test_ntsc_argument_is_working_properly(self):
        """ntsc argument value is properly passed to the ntsc
        attribute
        """
        r = Rate(ntsc=True)
    ...
    def test_ntsc_attribute_is_working_properly(self):
        """ntsc attribute is working properly"""
        r = Rate(ntsc=True)
        self.assertEqual(r.ntsc, True)
        r.ntsc = False
    ...
    def test_to_xml_method_is_working_properly(self):
        """to_xml() method is working properly"""
  ...
  <ntsc>FALSE</ntsc>
    ...
    def test_from_xml_method_is_working_properly(self):
        """from_xml() method is working properly"""
