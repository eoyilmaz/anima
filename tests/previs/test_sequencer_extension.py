...
class SequencerExtensionTestCase(unittest.TestCase):
    ...
    def test_create_shot_is_working_properly(self):
        """SequencerExtension.create_shot() is working properly"""
        s = pymel.core.createNode("sequencer")
        shot = s.create_shot(name="Test Shot", handle=14)
        self.assertIsInstance(shot, pymel.core.nt.Shot)

        self.assertEqual("shot1", shot.name())

        self.assertEqual("Test Shot", shot.shotName.get())

        self.assertEqual(14, shot.handle.get())

        self.assertEqual(s, shot.message.files()[0])

    def test_all_shots_property_is_working_properly(self):
        """all_shots property is working properly"""
        s = pymel.core.createNode("sequencer")
        s1 = s.create_shot("shot1")
        s2 = s.create_shot("shot2")
        s3 = s.create_shot("shot3")

        self.assertItemsEqual([s1, s2, s3], s.all_shots)

    def test_add_shot_with_an_unconnected_shot(self):
        """add_shot method is working properly for an
        unconnected shot
        """
        s = pymel.core.createNode("sequencer")
    ...
    def test_add_shot_will_add_handle_attributes_if_doesnt_exists(self):
        """add_shot method will add the handle attributes if the
        given shot doesn't have it
        """
        s = pymel.core.createNode("sequencer")
        s1 = pymel.core.createNode("shot")
        self.assertFalse(s1.hasAttr("handle"))

        s.add_shot(s1)

        self.assertTrue(s1.hasAttr("handle"))

        self.assertTrue(10, s1.handle.get())

    def test_add_shot_with_a_connected_shot(self):
        """add_shot method is working properly for a shot which
        ...
        self.assertEqual([shot1, shot2], s1.all_shots)

        self.assertEqual([shot3, shot4], s2.all_shots)

        s1.add_shot(shot3)
        self.assertEqual([shot1, shot2, shot3], s1.all_shots)

        self.assertEqual([shot4], s2.all_shots)

        s1.add_shot(shot4)
        self.assertEqual([shot1, shot2, shot3, shot4], s1.all_shots)

        self.assertEqual([], s2.all_shots)

    def test_set_sequence_name_is_working_properly(self):
        """set_sequence_name is working properly"""
        s1 = pymel.core.createNode("sequencer")
        ...
        self.assertEqual(s1.sequence_name.get(), "Test Sequence")

    def test_get_sequence_name_is_working_properly(self):
        """get_sequence_name is working properly"""
        s1 = pymel.core.createNode("sequencer")
        self.assertFalse(s1.hasAttr("sequence_name"))
        ...
        self.assertEqual(s1.get_sequence_name(), "Test Sequence")

    def test_get_sequence_name_will_create_sequence_name_attribute_if_missing(self):
        """get_sequence_name will create the attribute if it is
        missing
        """
        s1 = pymel.core.createNode("sequencer")
        self.assertFalse(s1.hasAttr("sequence_name"))
    ...
    def test_duration_property_is_working_properly(self):
        """duration property is working properly"""
        sm = pymel.core.PyNode("sequenceManager1")
        seq1 = sm.create_sequence("sequence1")
    ...
    def test_manager_property_is_working_properly(self):
        """manager property is working properly"""
        sm = pymel.core.PyNode("sequenceManager1")
