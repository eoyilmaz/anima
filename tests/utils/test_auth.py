...
def test_authenticate_with_stalker_called_only_when_enable_ldap_authentication_is_false(
    ldap_server, create_test_db
):
...
def test_get_user_name_from_ldap(ldap_server, create_test_db):
    """getting user name from ldap is working properly"""
...
def test_get_user_groups_from_ldap(ldap_server, create_test_db):
    """getting user groups from ldap is working properly"""
    login = "pipeline"
    password = "password"
    from anima import defaults
    ...
    assert result == ["GPU Users Admin", "Administrators"]


def test_create_user_with_ldap_info_create_proper_stalker_user(
    ldap_server, create_test_db
):
    """create_user_with_ldap_info will create a proper Stalker user"""
    login = "pipeline"
    password = "password"
    from anima import defaults
...
def test_create_user_with_ldap_info_create_proper_stalker_user_with_groups(
    ldap_server, create_test_db
):
    """create_user_with_ldap_info will create a proper Stalker user"""
    login = "pipeline"
...
def test_authenticate_with_stalker_and_ldap_will_always_authenticate_with_ldap(
    ldap_server, create_test_db
):
    """anima.utils.authenticate() function will authenticate
...
def test_authenticate_with_stalker_and_ldap_authenticates_an_existing_ldap_user(
    ldap_server, create_test_db, monkeypatch
):
    """anima.utils.authenticate() function will authenticate a
...
def test_authenticate_with_stalker_and_ldap_authenticates_a_existing_ldap_user_creates_a_stalker_user(
    ldap_server, create_test_db, monkeypatch
):
    """anima.utils.authenticate() function will authenticate a
...
def test_authenticate_updates_user_password_if_stalker_fails_but_ldap_successes(
    ldap_server, create_test_db, monkeypatch
):
    """anima.utils.authenticate() will update the user password
    ...
    new_user = User(
        ...
        email="pipeline@somedifferent.com",
        name="Pipeline",
    )
