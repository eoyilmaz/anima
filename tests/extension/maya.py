...
class AttributeExtensionsTester(unittest.TestCase):
    ...
    def test_next_available_extension_is_working_properly(self):
        """Attribute.next_available extension is working properly"""
        self.assertIsNotNone(pm.general.Attribute.next_available)

    def test_next_available_with_no_previous_connections(self):
        """Attribute.next_available will work properly for an array
        ...
        self.assertIsInstance(attr, pm.general.Attribute)

        self.assertEqual(
            sequence_manager.sequences[0], sequence_manager.sequences.next_available
        )

    def test_next_available_with_no_multi_attribute(self):
        """attribute itself will be returned when it is not an
        multi attribute
        """
        time = pm.ls(type=pm.nt.Time)[0]
        self.assertEqual(time.outTime.next_available, time.outTime)

    def test_next_available_with_all_connected_attribute(self):
        """next available attribute will be returned when no
        ...
        attr = sequence_manager.sequences.next_available
        self.assertIsInstance(attr, pm.general.Attribute)
        self.assertEqual(3, sequence_manager.sequences.next_available.index())
