...
class QProgressBarWrapper(ProgressControllerBase):
    ...
    def __init__(self, progress_bar=None, minimum=0.0, maximum=100.0, value=0.0):
...
class PublisherElement(object):
    ...
    def create(self, parent=None):
        ...
        # Create Icons
        self.publisher_state_ok_icon = parent.style().standardIcon(
            QtWidgets.QStyle.SP_DialogYesButton
        )
        self.publisher_state_not_ok_icon = parent.style().standardIcon(
        ...
        self.progress_bar_manager = QProgressBarWrapper(
            progress_bar=self.progress_bar, minimum=0, maximum=100.0, value=0.0
        )
        ...
        QtCore.QObject.connect(
            self.check_push_button, QtCore.SIGNAL("clicked()"), self.run_publisher
        )

        QtCore.QObject.connect(
            self.help_push_button, QtCore.SIGNAL("clicked()"), self.show_publisher_docs
        )

        QtCore.QObject.connect(
            self.fix_push_button, QtCore.SIGNAL("clicked()"), self.run_fix_definition
        )
    ...
    def run_fix_definition(self):
        ...
        if m.clickedButton() == m.defaultButton():
            ...
            try:
                from anima.dcc.maya import publish
        ...
        if self.publisher:
            ...
            error = str(
                "".join(
                    [
                        i
                        for i in stringify(self.publisher_state_label.toolTip())
                        if ord(i) < 128
                    ]
                )
            )
    ...
    def state(self, state):
        ...
        if state:
            self._set_label_icon(
                self.publisher_state_label, self.publisher_state_ok_icon
            )
            self.publisher_state_label.setToolTip(self.passing_text)
            self.publisher_name_label.setStyleSheet("color: ;")
        else:
            self._set_label_icon(
                self.publisher_state_label, self.publisher_state_not_ok_icon
            )
...
class MainDialog(AnimaDialogBase, QtWidgets.QDialog):
    ...
    def closeEvent(self, event):
        ...
        if answer == QtWidgets.QMessageBox.Yes:
            ...
            if (
                self.version.is_latest_published_version()
                and self.version.absolute_full_path == "."
                and self.version.extension == ""
                and not self.version.created_with
            ):  # this means the last published version is empty
    ...
    def check_all_publishers(self):
        ...
        if current_time - self.last_run_date > 5:
            for publisher in self.publishers:
                ...
                self.scroll_area.ensureWidgetVisible(publisher.check_push_button)
                publisher.run_publisher()
    ...
    def check_publisher_states(self):
        ...
        if self.publishers:
            self.update_publisher_total_duration_info()
            if all([publisher.state for publisher in self.publishers]) and self.version:
