...
class MediaManager(object):
    ...
    def upload_version(self, task, file_object, variant_name=None, extension=""):
        ...
        Args:
            task (stalker.Task): The task that a version is uploaded to. Should
                be an instance of :class:`.Task` class.
            file_object (file): A file like object holding the content of the
                version.
            variant_name (str): A string showing the the variant name of the
                Version. If skipped defaults.version_variant_name will be used.
...
def update_dependencies_in_duplicated_hierarchy(task):
    ...
    for dependent_task in task.depends_on:
        if hasattr(dependent_task, "duplicate"):
            ...
            duplicated_task.depends_on.append(dependent_task.duplicate)
        else:
            logger.debug("there is no duplicate!")
            duplicated_task.depends_on.append(dependent_task)
...
def check_task_status_by_schedule_model(task):
    ...
    if task.is_leaf and task.schedule_model == "duration":
        depends_on_tasks_cmpl = True
        for dependent_task in task.depends_on:
            if dependent_task.status is not status_cmpl:
                depends_on_tasks_cmpl = False
                break

        if depends_on_tasks_cmpl:
...
def partial_task_query(parent_task=None):
    ...
    """
    inner_tasks = aliased(Task)
    users_as_resource = aliased(User, flat=True)
    subquery = DBSession.query(Task.id).filter(Task.id == inner_tasks.parent_id)
    query = (
        DBSession.query(
            Task.id,
...
def get_unique_variant_names(task_id, include_reprs=False):
    """Return the unique variant names for the given task.

    Args:
        task_id (int): The task id.
        include_reprs (bool): Including representations (variants with "@" in
            their name). By default this is False.

    Returns:
        list: A list of strings of unique variant names.
    """
