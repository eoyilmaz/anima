...
def get_maya_main_window():
    ...
    if ptr is not None:
        ...
        if IS_PYSIDE():
            ...
            return shiboken6.wrapInstance(ptr_conv_func(ptr), QtWidgets.QWidget)
        elif IS_PYQT4():
            import sip
...
class Maya(DCCBase):
    ...
    def __init__(self, version=None):
        ...
        self._prepare_workspace_file_content()

    def _prepare_workspace_file_content(self):
        """Prepare the workspace file content."""
        self.maya_workspace_file_content = self._get_workspace_file_content()

    @classmethod
    @lru_cache()
    def _get_workspace_file_content(cls) -> str:
        """Return the workspace file content.

        Returns:
            str: The workspace file content.
        """
        workspace_file_path = os.path.join(DATA_PATH, "workspace.mel")
    ...
    def open(
        ...
        try:
            ...
            # disabled the following because it was causing the workspace
            # to revert to the previous one even when the document has
            # opened successfully
