...
class ConformerUI(object):
    ...
    def _setup_ui(self):
        ...
        self.seq_combo_box.currentIndexChanged.connect(
            partial(self.seq_combo_box_changed)
        )
        ...
        self.scene_combo_box.currentIndexChanged.connect(
            partial(self.scene_combo_box_changed)
        )
        ...
        self.shot_in_combo_box.currentIndexChanged.connect(
            partial(self.shot_in_combo_box_changed)
        )
        ...
        self.shot_out_combo_box.currentIndexChanged.connect(
            partial(self.shot_out_combo_box_changed)
        )
        ...
        self.filter_statuses_check_box.stateChanged.connect(
            partial(self.filter_statuses_check_box_changed)
        )
        ...
        self.task_name_combo_box.currentIndexChanged.connect(
            partial(self.task_name_combo_box_changed)
        )
        ...
        self.record_in_check_box.stateChanged.connect(
            partial(self.record_in_check_box_changed)
        )
        ...
        self.slated_check_box.stateChanged.connect(
            partial(self.slated_check_box_changed)
        )
    ...
    def seq_combo_box_changed(self, *args):
        ...
        if not stalker_seq:
            ...
            all_scenes = (
                Task.query.filter(Task.parent == stalker_seq)
    ...
    def scene_combo_box_changed(self, *args):
        ...
        if project:
            ...
            if not scene:
                ...
                shots = (
                    Shot.query.filter(Shot.sequences.contains(seq))
                    .order_by(Shot.name)
                    .all()
                )
                for shot in shots:
    ...
    def shot_in_combo_box_changed(self, *args):
        ...
        if not shot_in:
            ...
            for shot in shots:
                if (
                    scene in shot.parents
                    and int(shot.name.split("_")[-1]) >= shot_in_num
                ):
    ...
    def get_shots_from_ui(self):
        ...
        if not stalker_project:
            QtWidgets.QMessageBox.critical(
                self.parent_widget, "Error", "Please Select a Stalker Project."
            )
            return
        ...
        if self.seq_combo_box.currentText() == "ALL":
            ...
            if self.scene_combo_box.currentText() == "ALL":
                ...
                stalker_scene = self.scene_combo_box.itemData(
                    self.scene_combo_box.currentIndex()
                )
                if self.shot_in_combo_box.currentText() == "ALL":
                    for shot in stalker_seq.shots:
                        if (
                            stalker_scene in shot.parents
                        ):  # assume that a shot is always a parent of it's scene
                            shots.append(shot)
                else:
                    shot_in = self.shot_in_combo_box.itemData(
                        self.shot_in_combo_box.currentIndex()
                    )
                    shot_out = self.shot_out_combo_box.itemData(
                        self.shot_out_combo_box.currentIndex()
                    )
        ...
        if not shots:
            QtWidgets.QMessageBox.critical(
                self.parent_widget, "Error", "No Valid Shots found!."
            )
    ...
    def get_latest_output_path(
        self, shot, task_name, ext="exr", return_raw_values=False
    ):
            ...
            if os.path.isdir(main_dir):
                ...
                for dir_name in dir_names:
                    if (
                        ...
                        and os.path.basename(dir_name)[0] == "v"
                        and os.path.basename(dir_name)[1:].isdigit()
                        and len(os.path.basename(dir_name)) == 4
                    ):
        ...
        if shots:
            ...
            for shot in shots:
                ...
                if t_name == "Comp" and clip_path is None:  # look for Cleanup task
                    clip_path = self.get_latest_output_path(
                        shot, "Cleanup", ext=extension
                    )
                ...
                if (
                    t_name == "Comp"
                    and clip_path
                    and self.plus_plates_check_box.isChecked()
                ):
                    plate_path = self.get_latest_output_path(
                        shot, "Plate", ext=extension
                    )
                    if plate_path:
            ...
            if clip_path_list:
                ...
                if plate_path_list and self.plus_plates_check_box.isChecked():
                    print(
                        "CREATING + PLATES XML... Please Wait... ----------------------------"
                    )
                    self.clip_paths_to_xml(
                        plate_path_list, record_in_list, self.xml_path
                    )
    ...
    def conform_shots_new(
        self, shots, include_slates=False, use_current_timeline=False
    ):
        """conforms given Stalker Shot instances from UI to a Timeline for Resolve"""
        if shots:
            t_name = self.task_name_combo_box.currentText()
    ...
    def list_shot_update_status(self):
        ...
        if shots:
                ...
                if has_valid_status is True:
                    ...
                    try:
                        ...
                        if self.alpha_only_check_box.isChecked():
                            ...
                            latest_task_name = os.path.splitext(last_version.filename)[
                                0
                            ]
                            version_folder = latest_task_name.split("_")[-1]
                            file_paths = glob.glob(
                        ...
                        if modification_date >= query_date:
                            ...
                            if (
                                not self.alpha_only_check_box.isChecked()
                                or has_alpha is True
                            ):
            ...
            if update_list:
                ...
                self.updated_shot_list.addItem(
                    "No Updated Shots found after specified date / ui specs."
                )
