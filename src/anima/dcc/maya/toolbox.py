...
from anima.dcc.maya import auxiliary, camera_tools, publish
from anima.dcc.maya.animation import Animation
from anima.dcc.maya.general import General
from anima.dcc.maya.modeling import Modeling
from anima.dcc.maya.previs import Previs
from anima.dcc.maya.reference import Reference
from anima.dcc.maya.render import Render, MayaColorManagementConfigurator
from anima.dcc.maya.rigging import Rigging
...
def repeat_last(call_data):
    ...
    command = (
        'print \\"\\";python(\\"from anima.dcc.maya.toolbox import '
...
def repeated_callback(callable_, *args, **kwargs):
    ...
    the pm.repeatLast
    """
    return pm.Callback(repeat_last, [callable_, args, kwargs])
    ...
    for command in __commands__:
        ...
        if uitype == "button":
            ...
            for c in children:
                c_uitype = c.type()
                if (
                    c_uitype in ["button", "staticText"]
                    and search_text in c.getLabel().lower()
                ):
...
def UI():
    ...
    toolbox_window = pm.window(window_name, wh=(width, height), title="Anima ToolBox")

    # the layout that holds the tabs
    main_form_layout = pm.formLayout("main_form_layout", nd=100, parent=toolbox_window)
    ...
    pm.formLayout(
        ...
        attachNone=[(search_field, "bottom")],
        attachControl=[(main_tab_layout, "top", 0, search_field)],
    )

    with main_tab_layout:
        # ----- GENERAL ------
        general_column_layout = pm.columnLayout(
            "general_column_layout", adj=True, cal="center", rs=row_spacing
        )
        with general_column_layout:
            ...
            color.next()
            from anima.dcc.maya.general import unknown_plugin_cleaner_ui
        ...
        reference_columnLayout = pm.columnLayout(
            "reference_columnLayout", adj=True, cal="center", rs=row_spacing
        )
        with reference_columnLayout:
            ...
            pm.button(
                ...
                c=repeated_callback(Reference.select_reference_in_reference_editor),
            ...
            pm.button(
                ...
                c=repeated_callback(Reference.fix_student_license_on_references),
            ...
            pm.button(
                ...
                c=repeated_callback(Reference.fix_student_license_on_selected_file),
                ann=Reference.fix_student_license.__doc__,
                bgc=color.color,
            )
            ...
            pm.button(
                ...
                c=repeated_callback(Reference.generate_repr_of_all_references_caller),
        ...
        modeling_column_layout = pm.columnLayout(
            "modeling_column_layout", adj=True, cal="center", rs=row_spacing
        )
        with modeling_column_layout:
            color.reset()
            pm.button(
                "toggleFaceNormalDisplay_button",
                l="toggle face normal display",
                c=repeated_callback(pm.runtime.ToggleFaceNormalDisplay),
                ann="toggles face normal display",
                bgc=color.color,
            )
            pm.button(
                "reverseNormals_button",
                l="reverse normals",
            ...
            with pm.rowLayout(nc=4, adj=1):
                ...
                pm.intSliderGrp(
                    "smooth_edges_iteration_intField", v=100, min=0, max=100
                )
            ...
            with pm.rowLayout(nc=8, rat=(1, "both", 0), adj=1):
                ...
                pm.button(
                    "center_button",
                    l="C",
                    c=repeated_callback(Modeling.set_pivot, 0),
                    bgc=(0.8, 0.8, 0.8),
                )
                pm.button(
                    "minus_X_button",
                    l="-X",
                    c=repeated_callback(Modeling.set_pivot, 1),
                    bgc=(1.000, 0.500, 0.666),
                )
                pm.button(
                    "plus_X_button",
                    l="+X",
                    c=repeated_callback(Modeling.set_pivot, 2),
                    bgc=(1.000, 0.500, 0.666),
                )
                pm.button(
                    "minus_Y_button",
                    l="-Y",
                    c=repeated_callback(Modeling.set_pivot, 3),
                    bgc=(0.666, 1.000, 0.500),
                )
                pm.button(
                    "plus_Y_button",
                    l="+Y",
                    c=repeated_callback(Modeling.set_pivot, 4),
                    bgc=(0.666, 1.000, 0.500),
                )
                pm.button(
                    "minus_Z_button",
                    l="-X",
                    c=repeated_callback(Modeling.set_pivot, 5),
                    bgc=(0.500, 0.666, 1.000),
                )
                pm.button(
                    "plus_Z_button",
                    l="+X",
                    c=repeated_callback(Modeling.set_pivot, 6),
                    bgc=(0.500, 0.666, 1.000),
                )
            ...
            with pm.rowLayout(nc=7, rat=(1, "both", 0), adj=1):
                ...
                pm.button(
                    l="128",
                    c=repeated_callback(Modeling.set_texture_res, 128),
                    bgc=ColorList.colors[0],
                )
                pm.button(
                    l="256",
                    c=repeated_callback(Modeling.set_texture_res, 256),
                    bgc=ColorList.colors[1],
                )
                pm.button(
                    l="512",
                    c=repeated_callback(Modeling.set_texture_res, 512),
                    bgc=ColorList.colors[2],
                )
                pm.button(
                    l="1024",
                    c=repeated_callback(Modeling.set_texture_res, 1024),
                    bgc=ColorList.colors[3],
                )
                pm.button(
                    l="2048",
                    c=repeated_callback(Modeling.set_texture_res, 2048),
                    bgc=ColorList.colors[4],
                )
                pm.button(
                    l="4096",
                    c=repeated_callback(Modeling.set_texture_res, 4096),
            ...
            with pm.rowLayout(nc=6, adj=1):
                ...
                button_with = 40
                pm.radioButton("W", w=button_with, al="left", ann="World")
                pm.radioButton("L", w=button_with, al="left", ann="Local")
                pm.radioButton("UV", w=button_with, al="left", ann="UV")
                pm.radioButton("C", w=button_with, al="left", ann="Component", sl=1)
                pm.radioButton("T", w=button_with, al="left", ann="Topology")
            ...
            color.next()
            pm.button(
                "create_bbox_from_selection_button",
        ...
        rigging_columnLayout = pm.columnLayout(
            "rigging_columnLayout", adj=True, cal="center", rs=row_spacing
        )
        with rigging_columnLayout:
            ...
            with pm.rowLayout(nc=2, adj=1):
                ...
                def ik_fk_limb_rigger_callback():
                    subdivision = pm.intField(
                        "bendy_ik_fk_subdivision_count_field", q=1, v=1
                    )
            ...
            pm.button(
                ...
                c=repeated_callback(Rigging.set_clusters_relative_state, 1),
            ...
            def pin_controller_callback(color, *args):
                ...
                from anima.dcc.maya import rigging
            ...
            pm.button(
                "create_hair_from_curves_button",
            ...
            color.next()
            pm.button(
                "artPaintSkinWeightsTool_button",
            ...
            def skin_tools_ui_caller(*args):
                from anima.dcc.maya.rigging import SkinToolsUI
            ...
            pm.button(
                "toggle_local_rotation_axes_button",
            ...
            pm.button(
                "toggle_display_rotate_pivot_button",
            ...
            pm.button(
                "export_blend_connections_button",
        ...
        render_columnLayout = pm.columnLayout(
            "render_columnLayout", adj=True, cal="center", rs=row_spacing
        )
        with render_columnLayout:
            ...
            color.next()
            pm.button(
                "auto_convert_to_redshift_button",
            ...
            def set_shape_attribute_wrapper(attr_name, value):
                ...
                apply_to_hierarchy = pm.checkBox(
                    apply_to_hierarchy_checkBox, q=True, v=True
                )
                disable_undo = pm.checkBox(disable_undo_queue_check_box, q=True, v=True)

                Render.set_shape_attribute(
                    attr_name, value, apply_to_hierarchy, disable_undo
                )

            with pm.rowLayout(nc=3, rat=(1, "both", 0), adj=1):
                pm.text(
                    "renderThumbnailUpdate_text",
                    l="renderThumbnailUpdate",
                    bgc=color.color,
                )
                pm.button(
                    "set_renderThumbnailUpdate_ON_button",
                    l="ON",
                    c=repeated_callback(pm.renderThumbnailUpdate, 1),
                    bgc=(0, 1, 0),
                )
                pm.button(
                    "set_renderThumbnailUpdate_OFF_button",
                    l="OFF",
            ...
            color.next()
            pm.button(
                "assign_substance_textures_button",
            ...
            color.next()
            pm.button(
                "normalize_texture_paths_button",
            ...
            pm.button(
                "unnormalize_texture_paths_button",
            ...
            color.next()
            pm.button(
                "assign_random_material_color_button",
            ...
            pm.button(
                "randomize_material_color_button",
            ...
            color.next()
            pm.button(
                "convert_to_ai_image_button",
                l="To aiImage",
                c=repeated_callback(Render.convert_file_node_to_ai_image_node),
            ...
            pm.button(
                ...
                l="Reflection Curve",
                c=repeated_callback(Render.generate_reflection_curve),
                bgc=color.color,
            )
            ...
            pm.button(
                ...
                c=repeated_callback(Render.import_gpu_content),
                bgc=color.color,
            )
            ...
            with pm.rowLayout(nc=3, adj=3, bgc=color.color):
                source_driver_field = pm.textField(
                    text="S:", w=50, bgc=color.color, ann="Source Driver"
                )
                target_driver_field = pm.textField(
                    text="L:", w=50, bgc=color.color, ann="Target Driver"
                )
                pm.button(
            ...
            def lighting_scene_builder_callback(*args):
                from anima.dcc.maya import render
        ...
        previs_columnLayout = pm.columnLayout(
            "previs_columnLayout", adj=True, cal="center", rs=row_spacing
        )
        ...
        animation_columnLayout = pm.columnLayout(
            "animation_columnLayout", adj=True, cal="center", rs=row_spacing
        )
        with animation_columnLayout:
            color.reset()

            color.next()
            from anima.dcc.maya import picker

            pm.text(l="===== Object Picker =====")

            pm.button(
                "picker_setParent_button",
                l="Set Parent",
            ...
            color.next()
            from anima.dcc.maya import pivot_switcher

            pm.text(l="===== Pivot Switcher =====")
            pm.button(
                "oyPivotSwitcher_setupPivot_button",
                l="Setup",
            ...
            pm.button(
                "oyPivotSwitcher_switchPivot_button",
                l="Switch",
            ...
            pm.button(
                "oyPivotSwitcher_togglePivot_button",
                l="Toggle",
            ...
            pm.button(
                "bake_alembic_animations_button",
            ...
            with rowLayout:
                ...
                pm.button(
                    ann="Exports maya camera to nuke",
                    l="cam2chan",
                    w=70,
                    c=repeated_callback(
                        Animation.cam_2_chan, startButtonField, endButtonField
                    ),
                    bgc=color.color,
                )
            ...
            pm.textFieldButtonGrp(
                ...
                e=1,
                bc=repeated_callback(smooth_selected_keyframes_text_fbg_callback),
            )
            ...
            pm.textFieldButtonGrp(
                ...
                bc=repeated_callback(
                    Animation.smooth_component_animation, smooth_component_anim
                ),
            )
        ...
        with obsolete_columnLayout:
            ...
            color.next()
            pm.button(
                "connectFacingRatioToVCoord_button",
                l="connect facingRatio to vCoord",
                c=repeated_callback(Render.connect_facingRatio_to_vCoord),
            ...
            with pm.rowLayout(nc=3, rat=(1, "both", 0), adj=1):
                pm.text(
                    "miFinalGatherCast_text", l="miFinalGatherCast", bgc=color.color
                )
                pm.button(
                    "set_miFinalGatherCast_ON_button",
                    l="ON",
                    c=repeated_callback(
                        set_shape_attribute_wrapper, "miFinalGatherCast", 1
                    ),
                    bgc=(0, 1, 0),
                )
                pm.button(
                    "set_miFinalGatherCast_OFF_button",
                    l="OFF",
                    c=repeated_callback(
                        set_shape_attribute_wrapper, "miFinalGatherCast", 0
                    ),
                    bgc=(1, 0, 0),
                )
            ...
            with pm.rowLayout(nc=3, rat=(1, "both", 0), adj=1):
                pm.text(
                    "miFinalGatherReceive_text",
                    l="miFinalGatherReceive",
                    bgc=color.color,
                )
                pm.button(
                    "set_miFinalGatherReceive_ON_button",
                    l="ON",
                    c=repeated_callback(
                        set_shape_attribute_wrapper, "miFinalGatherReceive", 1
                    ),
                    bgc=(0, 1, 0),
                )
                pm.button(
                    "set_miFinalGatherReceive_OFF_button",
                    l="OFF",
                    c=repeated_callback(
                        set_shape_attribute_wrapper, "miFinalGatherReceive", 0
                    ),
                    bgc=(1, 0, 0),
                )
            ...
            with pm.rowLayout(nc=3, rat=(1, "both", 0), adj=1):
                pm.text(
                    "miFinalGatherHide_text", l="miFinalGatherHide", bgc=color.color
                )
                pm.button(
                    "set_miFinalGatherHide_ON_button",
                    l="ON",
                    c=repeated_callback(Render.set_finalGatherHide, 1),
                    bgc=(0, 1, 0),
                )
                pm.button(
                    "set_miFinalGatherHide_OFF_button",
                    l="OFF",
            ...
            color.next()
            pm.button(
                "convertToMRTexture_button",
                l="use mib_texture_filter_lookup",
                c=repeated_callback(Render.use_mib_texture_filter_lookup),
                ann=(
            ...
            color.next()
            pm.button(
                "oyAddToSelectedContainer_button",
            ...
            pm.button(
                ...
                c=repeated_callback(General.toggle_attributes, "displayHandle"),
            ...
            pm.button(
                ...
                c=repeated_callback(General.reference_selected_objects),
            ...
            pm.button(
                ...
                c=repeated_callback(General.dereference_selected_objects),
                ann="sets objects display override to reference",
                bgc=color.color,
            )
    ...
    if last_tab_index:
        pm.tabLayout(main_tab_layout, e=1, sti=last_tab_index)
