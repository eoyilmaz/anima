...
idname_template = "stalker.{}_{}_menu"
registered_menus = []
...
class StripGenerator(object):
    ...
    def add_output(self, full_path, name=None, channel=1):
        ...
        """
        logger.debug(f"adding output from: {full_path}")
        ...
        if output_type == "image":
            ...
            logger.debug(f"output_type is unknown: {output_type}")
    ...
    def storyboard(self, task):
        ...
        storyboard = (
            Task.query.filter(Task.parent == task)
    ...
    def previs(self, task):
        ...
        # get the previs task
        previs = (
            Task.query.filter(Task.parent == task).filter(Task.name == "Previs").first()
        )
...
class StalkerMenu(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        layout.menu(StalkerAddFromProjectMenu.bl_idname, text="Add From Project")


def draw_stalker_entity_menu_item(self, context):
    """Draw one menu item."""
    logger.debug(f"entity_id   : {self.stalker_entity_id}")
    logger.debug(f"entity_name : {self.stalker_entity_name}")
...
def draw_stalker_project_menu_item(self, context):
    ...
    all_seqs = (
        Sequence.query.filter(Sequence.project == project).order_by(Sequence.name).all()
    )
    for seq in all_seqs:
        idname = idname_template.format(seq.entity_type, seq.id)
...
def draw_stalker_sequence_menu_item(self, context):
    ...
    for scene in sorted(seq.children, key=lambda x: x.name):
        ...
        idname = idname_template.format(scene.entity_type, scene.id)
...
def draw_stalker_scene_menu_item(self, context):
    ...
    # Add Everything
    op = layout.operator(
        StalkerSceneAddEverythingOperator.bl_idname, text="Add Everything"
    )
    op.stalker_entity_id = scene.id
    op.stalker_entity_name = scene.name
    ...
    # Add Storyboard Only
    op = layout.operator(StalkerSceneAddStoryboardOperator.bl_idname, text="Storyboard")
    op.stalker_entity_id = scene.id
    op.stalker_entity_name = scene.name

    # Add Previs Only
    op = layout.operator(StalkerSceneAddPrevisOperator.bl_idname, text="Previs")
    op.stalker_entity_id = scene.id
    op.stalker_entity_name = scene.name
    ...
    # Add From Shots Menu
    idname = "{}{}".format(
        idname_template.format(scene.entity_type, scene.id),
...
def draw_stalker_scene_add_from_shots_menu_item(self, context):
    """Draw one scene/add from shots scene menu item."""
    ...
    op = layout.operator(
        StalkerSceneAddAllShotOutputsOperator.bl_idname, text="Add All Shot Outputs"
    )
    op.stalker_entity_id = scene.id
    op.stalker_entity_name = scene.name
    ...
    # Previs
    op = layout.operator(
        StalkerSceneAddAllShotPrevisOutputsOperator.bl_idname, text="Previs"
    )
    op.stalker_entity_id = scene.id
    op.stalker_entity_name = scene.name
    ...
    # Animation
    op = layout.operator(
        StalkerSceneAddAllShotAnimationOutputsOperator.bl_idname, text="Animation"
    )
    op.stalker_entity_id = scene.id
    op.stalker_entity_name = scene.name
    ...
    # Lighting
    op = layout.operator(
        StalkerSceneAddAllShotLightingOutputsOperator.bl_idname, text="Lighting"
    )
    op.stalker_entity_id = scene.id
    op.stalker_entity_name = scene.name
    ...
    # Comp
    op = layout.operator(
        StalkerSceneAddAllShotCompOutputsOperator.bl_idname, text="Comp"
    )
    op.stalker_entity_id = scene.id
    op.stalker_entity_name = scene.name
    ...
    shots_task = (
        Task.query.filter(Task.parent == scene).filter(Task.name == "Shots").first()
    )

    if shots_task:
        for shot in shots_task.children:
            # add a shot menu
            idname = idname_template.format(shot.entity_type, shot.id)
            layout.menu(idname)
...
def draw_stalker_shot_menu_item(self, context):
    ...
    op = layout.operator(
        StalkerShotAddAllTaskOutputsOperator.bl_idname, text="Add All Task Outputs"
    )
    ...
    # Previs
    op = layout.operator(StalkerShotAddPrevisOutputOperator.bl_idname, text="Previs")
    op.stalker_entity_id = self.stalker_entity_id
    op.stalker_entity_name = self.stalker_entity_name

    # Animation
    op = layout.operator(
        StalkerShotAddAnimationOutputOperator.bl_idname, text="Animation"
    )
    op.stalker_entity_id = self.stalker_entity_id
    op.stalker_entity_name = self.stalker_entity_name
    ...
    # Lighting
    op = layout.operator(
        StalkerShotAddLightingOutputOperator.bl_idname, text="Lighting"
    )
    op.stalker_entity_id = self.stalker_entity_id
    op.stalker_entity_name = self.stalker_entity_name

    # Comp
    op = layout.operator(StalkerShotAddCompOutputOperator.bl_idname, text="Comp")
    op.stalker_entity_id = self.stalker_entity_id
    op.stalker_entity_name = self.stalker_entity_name
...
class StalkerAddFromProjectMenu(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        for project in Project.query.order_by(Project.name).all():
            idname = f"stalker.{project.entity_type}_{project.id}_menu"
            layout.menu(idname)
...
class StalkerSceneAddEverythingOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        logger.debug(f"inside {self.__class__.__name__}.execute()")
        ...
        for task in scene.walk_hierarchy():
            if isinstance(task, Shot):
                ...
                animation_task = (
                    Task.query.filter(Task.parent_id == task.id)
                    .filter(Task.name == "Animation")
                    .first()
                )
...
def generate_op_class(
    entity, draw=draw_stalker_entity_menu_item, idpostfix="", label=None
):
    ...
    """
    idname = "{}{}".format(
        idname_template.format(entity.entity_type, entity.id), idpostfix
    )
    return type(
        idname,
        (bpy.types.Menu,),
...
def register():
    ...
    #
    for project in Project.query.order_by(Project.name).all():
        opclass = generate_op_class(project, draw=draw_stalker_project_menu_item)
        ...
        for seq in all_seqs:
            opclass = generate_op_class(seq, draw=draw_stalker_sequence_menu_item)
            bpy.utils.register_class(opclass)
            registered_menus.append(opclass)

            # for each sequence register a scene menu
            for sce in seq.children:
                opclass = generate_op_class(sce, draw=draw_stalker_scene_menu_item)
                bpy.utils.register_class(opclass)
                registered_menus.append(opclass)
        ...
        for shot in Shot.query.filter(Shot.project == project).all():
            opclass = generate_op_class(shot, draw=draw_stalker_shot_menu_item)
            bpy.utils.register_class(opclass)
            registered_menus.append(opclass)
