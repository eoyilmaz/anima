...
compiler_config = {
    "pyqt4": {"compiler": pyqt4_compiler, "postfix": "_UI_pyqt4.py"},
    "pyside": {
        "compiler": pyside_compiler,
        "postfix": "_UI_pyside.py",
    },
    "pyside2": {"compiler": pyside2_compiler, "postfix": "_UI_pyside2.py"},
}
    ...
    def compile(self, ui_file, output_path):
        if self.compiler:
            py_file_full_path = os.path.join(
                output_path, self.get_py_file_name(ui_file.full_path)
            )
...
class UIFile(object):
    ...
    def _validate_full_path(self, full_path):
        ...
        self.md5_file_full_path = os.path.join(self.path, self.md5_filename)
