...
def getChildFolders(path, return_full_path=False):
    ...
    childFolders = filter(os.path.isdir,
                          map(os.path.join, [path] * len(childFolders),
                              childFolders))
...
def getChildFiles(path, return_full_path=False):
    ...
    childFiles = filter(os.path.isfile,
                        map(os.path.join, [path] * len(childFiles),
                            childFiles))
...
def backupFile(full_path, maximum_backupCount=None):
    ...
    while True:
        ...
        backup_file_full_path = full_path + '.' + str(
            backup_no) + backup_extension
