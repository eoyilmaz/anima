...
def discover_env_vars(env_name=""):
    ...
    If the ``dcc_name`` argument is given it will initialize the environment
    variables registered under that DCC name.
    ...
    If "*" is given as the "dcc_name" (in the env.json file), all the variables
    will be registered regardless of what the ``dcc_name`` argument is given.
